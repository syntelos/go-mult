...
func (this Line) read(inl []byte) (Line) {
	...
	this.name = trim(inl[begin:end])
	{
		begin = class(inl,end,inz,ws)+1

		end = class(inl,begin,inz,id)+1
	}
	this.tag = trim(inl[begin:end])
	{
		begin = class(inl,end,inz,ws)+1

		end = class(inl,begin,inz,id)+1
	}
	this.code = trim(inl[begin:end])
	{
		begin = class(inl,end,inz,ws)+1

		end = class(inl,begin,inz,id)+1	
	}
	this.status = trim(inl[begin:end])
	{
		begin = class(inl,end,inz,ws)+1
	}
	this.description = rewrite(inl[begin:])

	this.cid = this.classid(this.tag,this.name)

	return this
}
/*
 * Upcase substrings delimited by token or punctuation.
 */
func Upcase(dst *strings.Builder, src_str string) {
...
/*
 * Camel substrings delimited by token or punctuation.
 */
func Camel(dst *strings.Builder, src_str string) {
	var src []byte = []byte(src_str)
	var src_len int = len(src)
...
func (this Line) classid (cl, id string) (string) {
	var ci *strings.Builder = new(strings.Builder)
...
func (this Line) print(order Order){
	...
	switch order {
	case Fwd:
		fmt.Printf("%s\t%s\t%s\n",this.code,this.name,this.tag)

	case Rev:
		fmt.Printf("%s\t%s\t%s\n",this.tag,this.name,this.code)

	default:
		fmt.Printf("%s\t%s\t%s\t%s\t%s\n",this.tag,this.name,this.code,this.status,this.description)
	}
}
...
/*
 */
const location_rel string = "multicodec_table.csv"
const location_doc string = "doc/multicodec_table.csv"
